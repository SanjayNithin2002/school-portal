...
const AddTimeTable = () => {
    ...
    const updateSubjectList = (std, sec, subject, teacher) => {
        let flag = -1;
        subjectList.map((subject1, index) => { if (subject1.standard === std && subject1.section === sec) { flag = index } })
        if (flag !== -1) {
            setSubjectList(prev => {
                const updated = [...prev];
    ...
    const handleSubmit = () => {
            ...
            timetableData2.map((info)=>{
                info.slot.map((slot)=>{
                    class1.docs.filter((class2)=>class2.standard===info.standard && class2.section===info.section && class2.subject===slot.subject).map((class2)=>{
                        if(class2.teacher)
                        if(class2.teacher._id===slot.teacher._id){
    ...
    if (edit) {
        ...
        std.map((item) => {
            let sec = Array.from(new Set(class1.docs.filter((item1) => item1.standard === item).map((obj) => obj.section)));
            sec.map((item1) => {
                timetableData2.filter((item2) => item2.standard === item && item2.section === item1).map((item2, index2) => {
                        ...
                        <div className='col-lg-8'>
                            {
                                standard && section ?
                                                                    ...
                                                                    timetableData2.filter((item) => item.day === day && item.standard === standard && item.section === section).map((item, index) => (
                                                                        ...
                                                                        item.slot.map((slot, index1) => (
                                                                            <tr>
                                                                                            ...
                                                                                            <td>
                                                                                                {
                                                                                                    ...
                                                                                                    <SelectPicker value="" onChange={(value) => { checkTeacher(timetableData2.findIndex((item3) => item3.day === day && item3.standard === standard && item3.section === section), index1, value); setEdit1(true); }} data={displaySubject} />
                                                                                                }
                                                                                            </td>
                                                                                    }
                                                                                </>
                                                                                <td>
                                                                                    {
                                                                                        ((slot.subject || slot.teacher) && slot.teacher !== "Break") ? <button className='btn btn-primary btn-sm' onClick={() => { updateSlotDetails(timetableData2.findIndex((item3) => item3.day === day && item3.standard === standard && item3.section === section), index1, null, null); setEdit1(true); }}>Clear</button> : <>No Action</>
                                                                                    }
                                                                                </td>
                                                                            </tr>
                                                                        ))
                                    ...
                                    <div style={{ display: "flex", justifyContent: "center", alignItems: 'center', height: "30vh" }} ><span style={{ backgroundColor: "darkgray", padding: "10px 5px", fontSize: "18px", fontWeight: "bold" }}><FontAwesomeIcon icon={solid.faExclamationTriangle} />&emsp;Select the class to edit the timetables</span></div>
                            }
                        </div>
                                                        ...
                                                        <table className='timetable-table' cellSpacing="20px">
                                                            {
                                                                Array.from(new Set(class1.docs.filter((item1) => item1.standard === item).map((obj) => obj.section))).map((item2) => (
                                                                    <tr onClick={() => { setStandard(item); setSection(item2); }} className='timetable-row'>
