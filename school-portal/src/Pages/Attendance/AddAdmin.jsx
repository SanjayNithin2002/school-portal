...
const AddTeacher = (props) => {
    ...
    useEffect(() => {
        if(fetchStatus){
            ...
            dispatch(getAdminAttendances("/PostTeacherAttendance", navigate, { date: props.date }));
        }
    ...
    const handleUpdate = () => {
        ...
        if (request1.length > 0) {
            dispatch(updateAdminAttendance("/PostTeacherAttendance", navigate, request1, { date: props.date }));
        }
        if (request2.length > 0) {
            dispatch(postAdminAttendance("/PostTeacherAttendance", navigate, { date: props.date, attendances: request2 }))
        }
        if(request3.length > 0) {
            dispatch(deleteAdminAttendance("/PostTeacherAttendance", navigate, request3));
        }
    }
    ...
    const handleSubmit = () => {
        if (attendance) {
            ...
            console.log(att);
            dispatch(postAdminAttendance("/PostTeacherAttendance", navigate,{ date: props.date, attendances: att }))
            handleBack();
        }
    }
    ...
    if (attendance && attendances && attendances.docs.length > 0 && edit) {
        ...
        attendances.docs.filter((item) => checkDate(item.date, props.date)).map((item) => {
            att.push({ admin: item.admin._id, status: item.status === 'Present' ? "Present" : "Absent" });
            return true;
        })
        attendance.map((item) => {
